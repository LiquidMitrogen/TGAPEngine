...
GLubyte *LoadTGAImage(char *filename, TARGAINFO *info, bool ignoreHeaders)
{
	...
	if(plik)
	{
		...
		if ((ignoreHeaders == true) || (memcmp(TGAHeader, TGACompare, sizeof(TGAHeader)) == 0))	// Nag³ówek identyczny?
		{
			...
			if(info->width>0 && info->height>0 && (info->bpp==24 || info->bpp==32 || info->bpp==8))
			{
...
//
//	// ³adowanie pliku
//	bits = LoadTGAImage(filename, &info,false);	// Próba wczytania tekstury
